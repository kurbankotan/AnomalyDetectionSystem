...
def baseline_model():
	# create model
	model = Sequential()
	model.add(Dense(23, input_dim=41, activation='relu'))
...
con_matrix = confusion_matrix(testLabel.argmax(axis=1), pred.argmax(axis=1), labels= list(le.transform(Classes))) #, labels=Classes
